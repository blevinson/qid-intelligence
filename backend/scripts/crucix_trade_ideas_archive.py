...
def _parse_episode(episode: dict) -> dict[str, Any] | None:
    ...
    # Primary path: the bridge writes a JSON episode_body (EpisodeType.json)
    # with keys type/direction/ticker/confidence/thesis/risk/horizon/
    # signals/title (see crucix_bridge.ingest_idea). Parse it directly.
    ...
    if data is not None and ("direction" in data or "type" in data or "thesis" in data):
        direction = (data.get("direction") or data.get("type") or "")
        direction = direction.upper() if isinstance(direction, str) and direction else None
        # Skip outcome episodes that may share the name prefix.
        if direction == "IDEA_OUTCOME":
            return None
        confidence = data.get("confidence")
        confidence = confidence.upper() if isinstance(confidence, str) and confidence else None
        signals = data.get("signals")
        signals_json = json.dumps(signals) if signals is not None else None
        return {
            "time": idea_dt,
            "idea_name": name,
            # Prefer the JSON ticker (already uppercased by the bridge) but
            # fall back to the name-derived ticker.
            "ticker": (data.get("ticker") or ticker or "").upper() or ticker,
            "direction": direction,
            "confidence": confidence,
            "title": data.get("title") or None,
            "thesis": data.get("thesis") or None,
            "risk": data.get("risk") or None,
            "horizon": data.get("horizon") or None,
            "signals": signals_json,
        }

    # Legacy path: older episodes used a narrative text body. Keep the regex
    # parse so the cron still backfills pre-JSON episodes.
