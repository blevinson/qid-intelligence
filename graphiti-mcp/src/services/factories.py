...
class LLMClientFactory:
    ...
    def create(config: LLMConfig) -> LLMClient:
        ...
        match provider:
            ...
            case 'claude_code':
                # Subscription path — uses the local `claude` CLI via the
                # Anthropic Agent SDK with OAuth credentials at
                # $HOME/.claude/.credentials.json. No API key required.
                # Adapter ships with qid-intelligence/backend/llm/.
                logger.info('Creating ClaudeCode (subscription) LLM client')
                from llm import ClaudeCodeLLMClient  # type: ignore[import-not-found]
                from graphiti_core.llm_client.config import LLMConfig as CoreLLMConfig

                llm_config = CoreLLMConfig(
                    model=config.model or 'claude-haiku-4-5',
                    small_model=config.model or 'claude-haiku-4-5',
                    temperature=config.temperature,
                    max_tokens=config.max_tokens,
                )
                return ClaudeCodeLLMClient(config=llm_config)
...
class EmbedderFactory:
    ...
    def create(config: EmbedderConfig) -> EmbedderClient:
        ...
        match provider:
            ...
            case 'tei':
                # In-cluster HuggingFace text-embeddings-inference
                # (BGE-M3 by default, 1024-dim). Adapter from qid-intelligence.
                logger.info('Creating TEI embedder')
                from llm import TEIEmbedder, TEIEmbedderConfig  # type: ignore[import-not-found]
                tei_cfg = TEIEmbedderConfig()
                return TEIEmbedder(config=tei_cfg)
